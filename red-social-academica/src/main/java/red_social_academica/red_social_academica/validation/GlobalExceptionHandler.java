...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ApiError> handleValidationExceptions(MethodArgumentNotValidException ex) {
        ...
        Map<String, String> errores = ex.getBindingResult() // Obtiene el resultado de la validación
                ...
                .map(error -> (FieldError) error) // Convierte a FieldError para acceder a los métodos específicos de
                                                  // campo
                ...
                .collect(Collectors.toMap(
                        ...
                        (error1, error2) -> error2 // Maneja conflictos de clave (campo duplicado) manteniendo el último
                                                   // error
                ));
    ...
    // 3. Maneja errores de validación a nivel de parámetros (@RequestParam,
    // @PathVariable)
    ...
    public ResponseEntity<ApiError> handleGenericException(Exception ex, WebRequest request) {
        // En producción, podrías querer loggear la excepción pero no mostrarla al
        // cliente
        ApiError apiError = new ApiError(
    ...
    // 8. Excepciones personalizadas de negocio de cada entidad
    ...
    public ResponseEntity<ApiError> handleBusinessException(BusinessException ex) {
        ...
        try {
            detalles = Arrays.stream(raw.replace("{", "").replace("}", "").split(", "))
                    .map(e -> e.split("="))
                    .collect(Collectors.toMap(
                            e -> e[0].trim(),
                            e -> e[1].trim()));
    ...
    // 9. Maneja errores de deserialización o JSON mal formado
    ...
    public ResponseEntity<ApiError> handleHttpMessageNotReadable(HttpMessageNotReadableException ex) {
        ...
        if (detalles != null && detalles.contains("Cannot deserialize value of type")
                && detalles.contains("java.time.LocalDate")) {
    ...
    // 10. Maneja violaciones de integridad de datos (ej: clave duplicada)
    ...
    // 11. Maneja IllegalArgumentException para validaciones manuales
